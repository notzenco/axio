...
export function TerminalWorkspace({ notify, snapshot, task, terminal }: TerminalWorkspaceProps) {
  ...
  const { activeCount, busy, close, sessions, spawn, stop } = terminal;
    ...
    <section className="terminal-workspace" aria-labelledby="terminal-workspace-heading">
      <header className="terminal-workspace-header">
        ...
        <div className="terminal-launch-controls">
          ...
          <label><span>Instances</span><input type="number" min="1" max={Math.max(1, launchLimit)} value={displayCount} disabled={launchLimit === 0} onChange={(event) => setCount(normalizeTerminalCount(event.target.valueAsNumber, launchLimit))} /></label>
          <button className="primary-button" type="button" disabled={!isNative || busy || launchLimit === 0} onClick={() => void launch()}><Add20Regular />{busy ? "Launching…" : launchLimit === 0 ? "At capacity" : "Launch"}</button>
        </div>
      </header>
      <div className="terminal-scope-note"><strong>{task.title}</strong><span>Shared working directory: {snapshot.repository?.root}</span><span>{activeCount} of {MAX_TERMINAL_SESSIONS} active app-wide</span></div>
      {sessions.length > 0
